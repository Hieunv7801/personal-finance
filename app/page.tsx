...
import { AlertTriangle, ArrowDownToLine, Banknote, CircleDollarSign, Clock3, Coffee, Fuel, Gift, HomeIcon, House, Landmark, LayoutDashboard, LogOut, MoreHorizontal, Plus, SettingsIcon, ShoppingBag, Utensils } from "lucide-react";
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <aside className="sidebar"><div className="logo-row"><Logo /><div><strong>Money Flow</strong><span>Chu kỳ {settings.salary_day} → 14</span></div></div><nav>{nav.map(([k, l]) => <button key={k} className={tab === k ? "nav-active" : ""} onClick={() => setTab(k)}><NavIcon name={k} />{l}</button>)}</nav><button className="logout" onClick={signOut}>Đăng xuất</button></aside>
    <header className="mobile-account-bar"><strong>Money Flow</strong><button type="button" onClick={signOut}><LogOut size={18} aria-hidden="true" />Đăng xuất</button></header>
